...
- [ ] `TabGroup` 스키마에 `tags: string[]` 필드 추가
- [ ] `addTagToGroup(groupId, tag)` 함수 구현
- [ ] `removeTagFromGroup(groupId, tag)` 함수 구현
- 검증: 태그 추가/삭제 후 storage에 정상 반영되는지 확인

### Task 9-2: 태그 UI
- [ ] 그룹 카드에 태그 목록 표시 및 태그 추가/삭제 UI 구현
- 검증: 태그 추가 후 카드에 즉시 표시되는지 확인

### Task 9-3: 사이드바 탭 전환 (세션 / 태그)
- [ ] 사이드바 상단에 `세션` / `태그` 탭 UI 추가
- [ ] `세션` 탭: 기존 세션 빠른 이동 목록 표시
- [ ] `태그` 탭: 전체 태그 목록 표시
- 검증: 탭 클릭 시 사이드바 콘텐츠가 전환되는지 확인

### Task 9-4: 태그 필터링
- [ ] `태그` 탭에서 태그 클릭 시 해당 태그를 가진 세션만 표시
- [ ] 필터 해제 시 전체 세션 복원
