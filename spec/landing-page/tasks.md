...
- [ ] 그룹 카드 헤더에 Fold 토글 버튼 추가
- [ ] `.tab-list`에 `.is-folded` 클래스 토글로 개별 접기/펼치기 구현 (`toggleFoldGroup`)
- [ ] 헤더에 `#btn-fold-all` 버튼 추가
- [ ] `foldAll(fold)` 함수 구현 — 전체 접기/펼치기 및 버튼 레이블 전환
- [ ] Fold 상태에 맞는 CSS 스타일 추가 (`.tab-list.is-folded { display: none }`)
