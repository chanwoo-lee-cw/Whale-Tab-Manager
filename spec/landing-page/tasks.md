...
### Task 9-5: 태그 자동완성
- [x] 태그 입력 시 기존 태그 목록 기반 자동완성 UI 구현
- [x] 사용되지 않는 태그는 자동으로 정리 (태그 삭제 후 어떤 그룹에도 없으면 제거)
- 검증:
  - 태그 입력 중 기존 태그 후보가 표시되는지 확인
  - 모든 그룹에서 태그 제거 시 자동완성 후보에서 사라지는지 확인
...
### Task 10-1: 세션 드래그 로직
- [x] `moveGroupToIndex(srcGroupId, dstGroupId)` 함수 구현 — groups 배열 내 순서 변경 후 storage 저장
- 검증: 함수 호출 후 storage의 groups 순서가 올바르게 업데이트되는지 확인

### Task 10-2: 세션 드래그 UI
- [x] `.group-card`에 `draggable="true"` 속성 및 drag 이벤트 핸들러 추가
- [x] `.group-card`에 `dragover` / `drop` 이벤트 핸들러 추가 (카드 간 이동)
- [x] 드래그 중 `.is-dragging` 클래스, 드롭 위치 `.drag-over` 표시 CSS 추가
- [x] 드롭 후 `refreshGroupList` 호출로 변경 사항 즉시 반영
- [x] 탭/헤더 dragover에 세션 드래그 중 가드 추가 (_draggingGroupId 플래그)
